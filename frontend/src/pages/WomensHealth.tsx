...
const WomensHealth = () => {
  ...
  useEffect(() => {
    api.get("/womens-health/predictions").then(r => {
      const p = r.data.predictions;
      if (p) setPredictions({ nextPeriod: p.nextPeriod, ovulation: p.ovulationDay, fertileStart: p.fertileStart, fertileEnd: p.fertileEnd });
      setCycle(r.data);
    }).catch(() => {});
  }, []);
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { data } = await api.post("/womens-health", { lastPeriodDate: form.lastPeriodDate, cycleLength: +form.cycleLength, periodDuration: +form.periodDuration, symptoms: form.symptoms });
