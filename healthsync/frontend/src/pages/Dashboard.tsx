...
import { foodAPI, medicineAPI, wellnessAPI } from '@/services/api';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    foodAPI.getWeekly().then(r => {
      ...
      const today = data[data.length - 1]?.intake || 0;
      setTodayCalories(today);
    }).catch(() => {});
    ...
    medicineAPI.getAll().then(r => {
      const meds = r.data;
      const taken = meds.filter((m: any) => m.status === 'taken').length;
      setAdherence({ taken, total: meds.length });
    }).catch(() => {});
  }, [user]);

  const adherencePct = adherence.total > 0 ? Math.round((adherence.taken / adherence.total) * 100) : 85;
  ...
  const displayCalories = todayCalories || 1850;
    ...
    <AppLayout title="Dashboard">
      <motion.div className="space-y-6" initial="initial" animate="animate">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard index={2} icon={Zap} title="Energy Level" value="7/10" subtitle="Above average" trend="up" trendValue="Better than last week" />
